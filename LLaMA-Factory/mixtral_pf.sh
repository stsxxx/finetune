...
cp -f ../copy_for_prof/modeling_mixtral.py /home/stilex/.local/lib/python3.8/site-packages/transformers/models/mixtral
cp -f ../copy_for_prof/trainer.py /home/stilex/.local/lib/python3.8/site-packages/transformers
cp -f ../copy_for_prof/workflow.py ./src/llmtuner/train/sft
cp -f ../copy_for_prof/util.py ./src/llmtuner/data

config_file_path="/data3/llama_model/yuchen/hub/models--mistralai--Mixtral-8x7B-v0.1/snapshots/985aa055896a8f943d4a9f2572e6ea1341823841/config.json"
...
#     cp -f /home/stilex/copy_for_tune/workflow.py /home/stilex/dst/LLaMA-Factory/src/llmtuner/train/sft
...
#         --cutoff_len 1024 \
#         --lora_rank 16 \
#         --bf16 > /home/stilex/dst/LLaMA-Factory/mixtral1/math_throughput_b${batch_size}_dense.txt 
#     wait
# done
...
for batch_size in "${batch_sizes[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=0 /usr/local/cuda-11.8/nsight-compute-2022.3.0/ncu  --target-processes all --nvtx --nvtx-include "Backward" --replay-mode application --devices 0 -f -o ./profile_data/mixtral/ncu/sparseb${batch_size}_${seq_len} python src/train_bash.py \
        --stage sft \
        --do_train \
        --model_name_or_path mistralai/Mixtral-8x7B-v0.1 \
        --dataset math \
        ...
        --cutoff_len 1024 \
        --lora_rank 16 \
        --bf16 > ./profile_data/mixtral/ncu/sparseb${batch_size}_${seq_len}.txt 
...
for batch_size in "${batch_sizes[@]}"; do
    ...
    # Run the training command with the updated batch size
    CUDA_VISIBLE_DEVICES=0 /usr/local/cuda-11.8/nsight-compute-2022.3.0/ncu  --target-processes all --nvtx --nvtx-include "Backward" --replay-mode application --devices 0 -f -o ./profile_data/mixtral/ncu/denseb${batch_size}_${seq_len} python src/train_bash.py \
        --stage sft \
        --do_train \
        --model_name_or_path mistralai/Mixtral-8x7B-v0.1 \
        --dataset math \
        ...
        --cutoff_len 1024 \
        --lora_rank 16 \
        --bf16 > ./profile_data/mixtral/ncu/denseb${batch_size}_${seq_len}.txt 
