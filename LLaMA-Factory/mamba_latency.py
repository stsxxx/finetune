...
def read_log_file(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        for line in lines:
            self_attention_match = re.search(r'final layernorm time: (\d+\.\d+)', line)
            moe_match = re.search(r'moe time: (\d+\.\d+)', line)
            input_norm_match = re.search(r'rms rnorm time: (\d+\.\d+)', line)
            after_norm_match = re.search(r'mamba layer time: (\d+\.\d+)', line)
...
def process_directory(directory_path):
    for file_name in os.listdir(directory_path):
        if file_name.endswith('.txt'):
            ...
            avg_finallayernorm_time = calculate_average(self_attention_times)
            avg_mamba_time = calculate_average(after_attention_times)
            avg_moe_time = calculate_average(moe_times)
            avg_rms_norm_time = calculate_average(input_norm_times)
            ...
            # print(f"Forward Average input normalization Time: {avg_input_norm_time_f} seconds")
            # print(f"Forward Average Self-Attention Time: {avg_self_attention_time_f} seconds")
            # print(f"Forward Average MoE layer Time: {avg_moe_time_f} seconds")
            # print(f"Forward Average moe fully connected layer Time: {avg_moe_ffc_time_f} seconds")
            # print(f"Forward Average moe masked layer Time: {avg_moe_mask_time_f} seconds")
            # print(f"Forward Average post attention normlization Time: {avg_output_norm_time_f} seconds")
            ...
            print(f"Average moe Time: {avg_moe_time * 18} seconds")
            print(f"Average mamba Time: {avg_mamba_time * 18} seconds")
            print(f"Average rms Time: {avg_rms_norm_time * 18} seconds")
            print(f"Average final layernorm Time: {avg_finallayernorm_time} seconds")
